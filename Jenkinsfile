node {
    ...
    try {
        ...
        stage('production') {
            sh 'helm upgrade --install prod-db helm/postgresql --namespace=prod --set persistence.enabled=true,persistence.storageClass=standard,postgresUser=confy,postgresPassword=confy01,postgresDatabase=confy'
